module.exports = {
    ...
    process: function(nodeType, node) {
        ...
        // Min size of a group:
        var groupSize = this.getValue('sort-order-group-size') || 1;
        var syntax = this.getSyntax();
        // Number of declarations that should be divided into groups:
        var groups = {};
        ...
        var extendNode = function() {
            ...
            if (groups[extendedNode.groupIndex]) {
                groups[extendedNode.groupIndex] += 1;
            } else {
                groups[extendedNode.groupIndex] = 1;
            }
        ...
        if (sorted.length > 0) {
            for (i = sorted.length - 1, l = -1; i > l; i--) {
                ...
                if (prevNode &&
                    ...
                    groups[currentNode.groupIndex] >=  groupSize &&
                    sc0[0] && sc0[0][0] === 's' &&
                    (this.syntax === 'sass' ||
                    sc0[0][1].match(/\n/g) &&
                    sc0[0][1].match(/\n/g).length < 2)) {
